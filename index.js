...
async function startXeonBotInc() {
    ...
    XeonBotInc.sendMessage(...global.mynum, {text: `${callerId} Called Bot`})
    await sleep(5000)
...
//═══════[get profile pic]════════\\
                try {
                    ...
                    ppuser = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
                }
...
//═══════[get group dp]════════\\
                try {
                    ...
                    ppgroup = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
                }
...
Kon = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/284d9fadb9646dc110bf6.jpg&namegb=${encodeURIComponenent(metadata.subject)}&member=${encodeURIComponent(memb)}`)

                if (anu.action == 'add') {
                  if (antinum.includes(anu.id) & numcode ){
await XeonBotInc.groupParticipantsUpdate(anu.id, [num], 'remove')
await XeonBotInc.updateBlockStatus(num, 'block')
                  }
                  
                   if (welcome.includes(anu.id) & !numcode) {
                 const welmess = await XeonBotInc.sendMessage(anu.id, { image: Kon, contextInfo: { mentionedJid: [num] }, caption: `Welcome To ${metadata.subject} @${num.split("@")[0]}
                    
${readMore}Please introduce yourself and read the description...

Description: 
${metadata.desc}`} )
await sleep(90000)
XeonBotInc.sendMessage(anu.id, { delete: welmess.key})
